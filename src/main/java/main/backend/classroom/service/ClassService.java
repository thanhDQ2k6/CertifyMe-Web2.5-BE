...
public class ClassService {
    ...
    public List<StudentResponseDTO> getStudentsInClass(String classId, String status, String sort, String order) {
        ...
        Stream<StudentResponseDTO> stream = enrollments.stream().map(this::mapStudentDTO);

        if (status != null && !status.isBlank()) {
            stream = stream.filter(s -> status.equalsIgnoreCase(s.getStatus()));
        }

        Comparator<StudentResponseDTO> comparator = Comparator.comparing(StudentResponseDTO::getFullName);
    ...
    private ClassResponseDTO mapToDTO(ClassEntity clazz) {
        ...
        long studentCount = enrollmentRepository.countByClassEntity_ClassId(clazz.getClassId());
