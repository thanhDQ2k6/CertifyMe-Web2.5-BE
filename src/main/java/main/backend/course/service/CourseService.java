...
public class CourseService {
  ...
  public List<CourseListItemResponse> getAllPublicCourses() {
    return courseRepository
      ...
      .stream()
      .map(course ->
        CourseListItemResponse.builder()
  ...
  public CourseDetailResponse getCourseDetail(
    ...
    List<QuizAttempt> studentAttempts =
      quizAttemptRepository.findByStudentAndClass(
        studentId,
        clazz.getClassId()
      );

    List<CourseDetailResponse.QuizDTO> quizDTOs = quizzes
      .stream()
      .map(quiz -> {
        QuizAttempt bestAttempt = studentAttempts
          .stream()
    ...
    int completedQuizzes = (int) quizDTOs
      .stream()
      .filter(q -> "completed".equals(q.getStatus()))
      .count();
    int progress =
      totalQuizzes > 0 ? ((completedQuizzes * 100) / totalQuizzes) : 0;
    boolean isCompleted = "PASSED".equalsIgnoreCase(
    ...
    var response = CourseDetailResponse.builder()
      ...
      .endDate(
        clazz.getEndDate() != null ? clazz.getEndDate().toString() : null
      )
      .progress(progress)
      ...
      certificateRepository
        ...
        .ifPresent(cert ->
          response.setCertificate(
            CourseDetailResponse.CertificateDTO.builder()
              .verificationHash(cert.getCertificateHash())
              .blockchainInfo(
                Map.of(
                  "hash",
                  cert.getCertificateHash() != null
                    ? cert.getCertificateHash()
                    : "",
                  "block",
                  "",
                  "txHash",
                  cert.getTransactionHash() != null
                    ? cert.getTransactionHash()
                    : "",
                  "contract",
                  cert.getContractAddress() != null
                    ? cert.getContractAddress()
                    : ""
                )
              )
              .build()
